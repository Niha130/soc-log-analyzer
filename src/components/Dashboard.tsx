  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          label="Total Logs" value={metrics?.totalLogs ?? logs.length}
        ...
        />
        <StatCard
          label="Critical Threats" value={criticalCount}
          sub={`${highCount} high severity`}
          icon={<Shield size={26} />}
          color="bg-red-900/30 border-red-800 text-red-300"
        />
        <StatCard
          label="Resolved" value={metrics?.resolvedIncidents ?? 0}
          icon={<Clock size={26} />}
        ...
        <div className="lg:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          {loading ? (
            ...
            <div className="h-44 flex items-center justify-center text-gray-600 text-sm">
              Chart builds after first data fetch...
            </div>
          )}
        </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        ...
        {loading ? (
          <p className="text-gray-600 text-sm animate-pulse">Loading logs...</p>
        ) : recentLogs.length === 0 ? (
          ...
          <div className="space-y-1.5">
            {recentLogs.map(log => (
              <div key={log.id}
                className={`flex items-start gap-3 rounded-lg border px-3 py-2 text-xs ${SEV_BG[log.severity] ?? 'bg-gray-800 border-gray-700'}`}>
                <span className={`font-bold uppercase w-16 shrink-0 mt-0.5 ${SEV_COLOR[log.severity] ?? 'text-gray-400'}`}>
                  {log.severity}
                </span>
                <span className="text-gray-500 shrink-0 font-mono">{log.hostname}</span>
                <span className="text-gray-300 truncate flex-1">{log.message}</span>
                <span className="text-gray-600 shrink-0 ml-auto">{log.timestamp?.slice(11,19)}</span>
              </div>
            ))}
          </div>
        )}
            ...
            {alerts.slice(0, 5).map(alert => (
              <div key={alert.id}
                ...
                <p className="text-xs text-gray-500 mt-1">
                  <span className="font-mono text-blue-400">{alert.sourceIp}</span>
                  {' → '}{alert.hostname}
                  {alert.mitre && <span className="ml-2 text-purple-400">MITRE: {alert.mitre}</span>}
                </p>
              </div>
            ))}
          </div>
